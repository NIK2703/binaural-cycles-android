 ...
 *
 * Форма нужна не только для перекрытия: на ПОДЪЁМЕ (t=1) sin-ветвь даёт
 * НУЛЕВОЙ терминальный наклон у 1.0 — остаточная ошибка тайминга рампы
 * квадратична по недоходу, а не линейна, и стык «конец fade-in → полный
 * звук» не щёлкает даже при лаге шейпера. На СПАДЕ нижний конец защищён
 * не формой, а порядком операций «база 0 ДО closeShaper/pause» — cos-ветвь
 * там лишь усиливает крутизну хвоста (терминальный наклон −π/2 ≈ −1.57
 * против −1 у прямой), что для спада к уже погашенной базе безвредно.
 */
...
interface BinauralStream {
     ...
     *        штатную). Парный параметр к `fadeOutMsOverride` у [stop]: обе
     *        рампы одного перехода задаются одной константой, чтобы слышимая
     *        длина была одна для всех маршрутов.
     */
    fun start(
        onFullyStarted: () -> Unit,
    ...
    /**
     * ПЕРЕНАСТРОЙКА ЖИВОГО ПОТОКА: сменить конфиг/сдвиг оси на ТОМ ЖЕ треке.
     *
     * Единственный штатный путь смены настроек и скраба во время игры. Второго
     * AudioTrack при этом не создаётся вовсе — а значит не нужны ни второй
     * пакет, ни сторож утилизации уходящего потока, ни «момент тишины», который
     * приходилось вычислять опросом живого множителя шейпера.
     *
     * @return `false` — перенастройка на живом потоке невозможна (не PLAYING или
     *         сменилась частота дискретизации); вызывающий строит новый поток.
     */
    fun retune(newSpec: PlaybackSpec, onApplied: (Boolean) -> Unit): Boolean
