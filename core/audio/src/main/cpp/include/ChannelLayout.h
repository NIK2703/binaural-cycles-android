 ...
 *
 * Функция НЕПРЕРЫВНА: «перестановка каналов» — не дискретное событие, а
 * проход |beat| через ноль. Несущая и громкость при этом не меняются, фаза в
 * ухе непрерывна по построению (ухо продолжает слушать тот же осциллятор),
 * поэтому щелчка нет в принципе — именно им приходилось платить за
 * перестановку буфера.
 *
 * ШАГ 1 МИГРАЦИИ: константа +1. Значение пока не влияет ни на что — сначала
 * весь аудио-путь переводится на channelsAt(), и только следующим шагом
 * s(t) наполняется расписанием. Это изолирует «смену маршрутизации частот»
 * от «смены семантики»: звук на шаге 1 обязан быть бит-в-бит прежним.
 */
