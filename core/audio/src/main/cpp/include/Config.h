...
/**
 * МЁРТВАЯ ЗОНА трендовой дельты, Гц (абсолютный пол).
 *
 * Δbeat считается как разность двух разностей частот КАНАЛОВ:
 *   (upper(t+h) − lower(t+h)) − (upper(t−h) − lower(t−h)).
 * При постоянной частоте биений она тождественно ноль, но на float32 даёт
 * «дребезг» порядка 1e-4 Гц: каналы несут 200…2000 Гц, относительная
 * точность float32 ~1e-7, и вычитание двух близких величин оставляет именно
 * такой хвост. Детектор нулей без мёртвой зоны принимает дребезг за смены
 * знака: кривая с постоянными биениями (две точки с одинаковым beat — ровно
 * то, что получается двойным тапом в редакторе) давала больше 5000
 * «экстремумов» за сутки, и каналы менялись местами каждые 5–30 с.
 *
 * Нижняя граница выбрана на порядок выше шума и на два порядка ниже
 * реального сигнала: осмысленные суточные тренды дают Δbeat 0.01…0.2 Гц.
 */
constexpr float TREND_DELTA_EPSILON_HZ = 1e-3f;

/**
 * Доля мёртвой зоны от максимальной частоты канала кривой: шум вычитания
 * растёт вместе с величинами, из которых он получен. Для несущей 600 Гц
 * это 6e-4 Гц — меньше абсолютного пола, поэтому пол и работает по умолчанию;
 * вклад становится заметным на верхних несущих (2000 Гц → 2e-3 Гц).
 */
constexpr float TREND_DELTA_RELATIVE_EPSILON = 1e-6f;
