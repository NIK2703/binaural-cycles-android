...
public:
    ...
    /**
     * DATA-BAKED FADE-IN (альтернатива VolumeShaper на старте потока).
     *
     * Вызывать ДО первого generateAudioBuffer (в prepare(), до старта
     * писателя): после этого потребителем счётчика является только
     * аудио-нить — гонок нет.
     *
     * ОДИН РАЗ НА ПОТОК: огибающая ложится на первые durationMs
     * сгенерированного аудио (при необходимости через границу двух
     * пакетов, если первый короче), после чего счётчик обнуляется и
     * больше не применяется: ни перегенерация по кольцу, ни пакет
     * retune, ни продолжение после паузы повторного фейда не получают.
     */
    void setPendingFadeIn(int durationMs);
    ...
    /**
     * Значение UI-времени БЕЗ побочного эффекта (без записи кэша).
     *
     * Отдельная функция, а не «флаг» у [computeUiTimeSeconds], потому что
     * геттеры обязаны считать позицию по текущему якорю, а не читать кэш,
     * который мог протухнуть относительно своего стража (см. баг
     * «возобновление с 0:00», docs/handoff_anchor_zero_analysis_plan.md).
     * Кэш — это «заморозка для UI на паузе», а не источник истины.
     */
    float computeUiTimeSecondsUncached() const;

    // ======= Слышимая позиция кривой (пауза / возобновление) =======
    // Две разные оси времени:
    //   * ФРОНТИР ГЕНЕРАЦИИ — m_curveTimeSeconds. Продвигается в момент
    //     generateAudioBuffer сразу на весь пакет (до 60 мин), звук при этом
    //     ещё не вышел из буфера.
    //   * СЛЫШИМОЕ ВРЕМЯ — то, что микшер реально отдал в динамики. Его
    //     несёт AudioTrack.playbackHeadPosition; нативной стороне он виден
    //     только как аргумент, поэтому обе оси связываются здесь.
    // Пауза обязана фиксировать СЛЫШИМУЮ позицию: без неё указатель графика
    // и точка возобновления уезжают вперёд на недописанный остаток пакета.
    //
    // @param playedFrames    кадры, реально отданные микшером
    //                        (AudioTrack.playbackHeadPosition)
    // @param generatedFrames кадры, сгенерированные движком с начала потока
    // @return секунды суток, соответствующие последнему прозвучавшему сэмплу
    float computeAudibleTimeSeconds(int64_t playedFrames, int64_t generatedFrames) const;

    /**
     * ФРОНТИР ГЕНЕРАЦИИ (секунды суток): конец последнего сгенерированного
     * пакета — то есть правая граница аудио, которое УЖЕ посчитано.
     *
     * Отличие от getCurrentTimeOfDaySeconds(): тот отдаёт UI-время (плавная
     * экстраполяция по wall-clock внутри сгенерированного диапазона), а этот —
     * реальный край генерации. Разница между ними и есть недослушанный хвост.
     *
     * Нужен решателю возобновления: пакет не устарел, пока текущий момент
     * суток лежит внутри [слышимое время, фронтир] (см.
     * docs/analysis_resume_from_0_position.md).
     */
    float getCurveTimeSeconds() const { return computePlaybackTimeSeconds(); }

    /**
     * Заморозить UI-указатель графика на слышимой позиции (пауза).
     * Диапазон [t, t]: экстраполяция по wall-clock обнуляется, указатель стоит
     * ровно там, где звук остановился — пауза любой длительности не уводит
     * график вперёд к фронтиру генерации.
     */
    void freezeUiTimelineAt(float seconds);

    /**
     * Снять заморозку (возобновление). Указатель продолжает с той же слышимой
     * позиции и далее идёт по wall-clock, но не выбегает за фронтир уже
     * сгенерированного аудио — частоты на экране не опережают звук.
     */
    void resumeUiTimelineFrom(float seconds);
...
private:
    ...
    /**
     * Домножить начало буфера на sin²-огибающую data-baked fade-in.
     * Только аудио-нить (вызывается из generateAudioBuffer).
     */
    void applyPendingFadeIn(float* buffer, int frames);

    // Data-baked fade-in: пишется владельцем до старта писателя,
    // читается аудио-нитью; два счётчика после старта живут только
    // на аудио-ните.
    std::atomic<int> m_pendingFadeInSamples{0};
    int m_fadeInTotalSamples = 0;
    int m_fadeInConsumed = 0;
