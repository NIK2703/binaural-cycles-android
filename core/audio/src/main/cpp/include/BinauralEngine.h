...
private:
    ...
    // Seam-mask: версия конфига, использованная последним СГЕНЕРИРОВАННЫМ
    // пакетом. Расхождение с текущей s_cfgVersion => первый пакет после
    // setConfig получает косинусную маску 0->1 в начале (анти-щелчок).
    std::atomic<uint32_t> m_lastCfgVersionGenerated{0};
