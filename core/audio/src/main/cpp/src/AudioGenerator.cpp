...
// Огибающая ступеньки STEP-кривой и правило наложения обеих процедур
// (смена каналов × ступенька). Включает ChannelLayout.h сам.
#include "StepFade.h"
#include <algorithm>
#include <cmath>
#include <cstring>
...
void AudioGenerator::generateSolidBuffer(
    ...
    for (int i = 0; i < samples; ++i) {
        ...
        buffer[i * 2] = leftSample * leftNormAmp;
        buffer[i * 2 + 1] = rightSample * rightNormAmp;
...
void AudioGenerator::generateSolidBufferNeon(
    ...
    for (; i < neonEnd; i += 4) {
        ...
        float32x4_t vLeftAmps = vAmpL;
        float32x4_t vRightAmps = vAmpR;
    ...
    for (; i < samples; ++i) {
        ...
        const float leftAmp = leftAmplitude;
        const float rightAmp = rightAmplitude;
...
void AudioGenerator::generateSolidBufferSse(
    ...
    for (; i < sseEnd; i += 4) {
        ...
        __m128 vLeftAmps = vAmpL;
        __m128 vRightAmps = vAmpR;
...
GenerateResult AudioGenerator::generatePackage(
    ...
    // Затухание на ступеньках STEP-кривой — та же процедура, что у смены
    // каналов (затухание → тишина → нарастание вокруг T*), но центр стоит в
    // контрольной точке, где меняется удерживаемое значение. Выключено в
    // debug-режиме постоянной частоты: кривая там не звучит, и провал на её
    // ступеньках был бы ложным.
    const bool stepFadeActive = !constantFreq &&
        config.curve.interpolationType == InterpolationType::STEP &&
        config.stepFadeDurationMs > 0;
    ...
    // Амплитуда уха = нормализация по частоте × огибающая процедур
    // (procedureGainAt: смена каналов и ступенька STEP; затухание → тишина →
    // нарастание вокруг своего T*). Частоты при этом продолжают идти по
    // графику — огибающая множит только громкость, поэтому и «смена каналов»,
    // и скачок ступеньки слышны как пауза, а не как перепад частот.
    // При выключенных процедурах это два if и умножение на 1.
    auto earAmpsAt = [&](const FrequencyTableResult& f, float t) -> std::pair<float, float> {
        ...
        // Огибающая ОБЕИХ процедур разом: смена каналов × ступенька STEP.
        // При наложении они ПЕРЕМНОЖАЮТСЯ, поэтому тишина покрывает
        // объединение пауз, а каждое событие всё равно приходится на нуль —
        // см. procedureGainAt в StepFade.h.
        const float gain = procedureGainAt(config, t, stepFadeActive);
...
GenerateResult AudioGenerator::generatePackageNeon(
    ...
    for (const auto& segment : plan.segments) {
        ...
        if (SEG_LOG_ENABLED()) {
            ...
            // Фикс (Qwen, P2): учитываем амплитуду (endAmp),
            // иначе сравнение с фактическим first вводит в заблуждение:
            // сырой sin(phase) больше реального сэмпла в 1/amp раз.
                    float expectedFirstLeft = Wavetable::fastSin(state.leftPhase) * endLeftAmp;
            float expectedFirstRight = Wavetable::fastSin(state.rightPhase) * endRightAmp;
