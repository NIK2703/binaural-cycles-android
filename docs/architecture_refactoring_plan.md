...
### Понятие "Интервал обновления частот" (удалён из C++)

Параметр `frequencyUpdateIntervalMs` ранее использовался для:
- Определения размера lookup table: `SECONDS_PER_DAY / intervalSeconds`
- Определения рекомендуемого размера буфера
- Передаётся через все функции генерации (но фактически игнорировался)

**Статус: параметр полностью удалён из C++/JNI слоя** (см. "Порядок реализации").
...
3. ~~Фактически не используется~~: исправлено — параметр удалён из всех сигнатур генерации, JNI-методы `nativeSetFrequencyUpdateInterval` / `nativeGetRecommendedBufferSize` убраны
...
7. ✅ Обновить Kotlin слой (JNI-мосты параметра удалены; Kotlin-метод `setFrequencyUpdateInterval` остался как настройка длительности пакета)
8. ✅ Удалить настройки из UI (осталась только настройка "Интервал генерации буфера")
9. ⬜ Тестирование
10. ⬜ Документация

### Текущая архитектура пакетов

- Генерация аудио выполняется пакетами через `BufferPackagePlanner` (C++) и batch-API
  (`nativeSetBatchDurationMinutes` / `nativeGenerateBatch`).
- Пакет = настраиваемый интервал 1..60 минут, по умолчанию 10 минут.
- Движок дополнительно ограничивает буфер жёстким байтовым капом (`MAX_BUFFER_BYTES`),
  чтобы большие интервалы при высоком sample rate не приводили к OOM.
- Частоты в UI/уведомлении обновляются O(1) из lookup table независимо от длительности пакета.
