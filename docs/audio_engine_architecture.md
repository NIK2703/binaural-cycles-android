...
**Режим перестановки по тенденции графика (`ChannelSwapMode::TREND`):**

В этом режиме длительность SOLID-фазы не константа, а вычисляется динамически —
«время до ближайшей смены тренда» несущей частоты кривой. Желаемое состояние в
момент t определяется знаком производной несущей (конечная разность по
lookup-таблице, окно ±60 с):

- рост (`Δf > +ε`) → обычное расположение каналов (`swapped = false`);
- спад (`Δf < −ε`) → обратное расположение (`swapped = true`);
- плато (`|Δf| ≤ ε`, `kTrendDeadbandHz = 0.05 Гц`) → состояние не меняется
  (гистерезис против дребезга; единственная защита от частых смен — искусственных
  минимальных пауз между swap нет).

Смена тренда ищется сканированием кривой вперёд от текущей позиции с шагом
15 с (`kTrendScanStepSec`) на сутки вперёд; если переходов нет, SOLID
ограничивается 30 минутами (`kTrendMaxSolidMs`) до переоценки. Fade-цикл
(SOLID → FADE_OUT → PAUSE → FADE_IN) используется тот же, что и в таймерном
режиме; параметр channelSwapIntervalSec в TREND-режиме **не участвует**. В TREND SOLID
обрывается за `leadMs + P/2` до экстремума `T*`, поэтому прерывание потока (конец
FADE_OUT) приходится на `T* − P/2`, а пауза `[T*−P/2, T*+P/2]` центрируется ровно на
`T*` (при `P = 0` — прерывание ровно на `T*`, базовое поведение).
При debug virtual time (`timeScale > 1`) смещение по кривой конвертируется в
аудио-длительность делением на масштаб.

Дополнительно: при свежем старте воспроизведения расположение каналов сразу
выставляется по знаку тренда в точке старта (до первого сэмпла); при
возобновлении после паузы состояние корректируется — выключенный swap
принудительно возвращает прямое расположение, включённый TREND выравнивает
расположение по актуальному тренду.
