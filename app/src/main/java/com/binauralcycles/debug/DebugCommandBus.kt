...
internal val DEBUG_HELP = """
    ...
    --- ЧЕРНОВИК РЕДАКТОРА (headless, для docs/analysis_trend_swap_editor_phantom.md) ---
      Путь «новая предустановка → play в редакторе → правка графика»
      вызывами сервиса один в один, без тыканья в Compose-график.
      Что именно дошло до движка — в logcat тегом SWAPCFG.
    dn                       создать черновик (1 точка, как новый пресет)
    dstart                   зазвучать черновиком (= startDraft, id __draft__)
    dadd <сек|HH:MM> <н> <б> добавить точку и пушить в звук (= addEditingPoint)
    drelax <on|off>          режим расслабления черновика (виртуальные точки!)
    dsave <имя>              сохранить черновик и сделать активным (= createPreset)
    dshow                    черновик + настройки свапа/расслабления
    dstop                    остановить звук (черновик остаётся)
    swapwatch <сек>          опрос знака биений каждые 200 мс, лог — SWAPWATCH:
                             сколько РЕАЛЬНО раз каналы менялись местами
