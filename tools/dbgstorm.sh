...
#   2. ПОРЯДОК ПЕРЕНАСТРОЙКИ — «приседание → пакет перестроен → ПОДНЯТ»:
#      каждый провал закрывается подъёмом ТОГО ЖЕ трека строго после
#      пересборки пакета на нити писателя. Кроссфейд и маркер `ТИШИНА`
#      удалены (§4.4 документа, единый retune-путь), смешение тут — дефект,
#      а не выбор маршрута.
#   3. МАРШРУТИЗАЦИЯ — единый путь retune() на одном треке: все причины
#      смены звука (SCRUB/SETTINGS/PRESET_SWITCH/SAMPLE_RATE) идут через
#      него, поэтому считается число приседаний, а не вердиктов когерентности.
...
report() {
    ...
    local requests_all requests retunes retunes_by_reason rebuilt lifted
    local swap recreated series waits
    requests_all=$(count "requestHandoff: spec#")
    requests=$(count "requestHandoff: spec#[0-9]+ в очередь .*reason=$reason")
    # ОДИН трек: «переход» — это приседание на живом потоке, пересборка
    # пакета на нити писателя и подъём ТОГО ЖЕ трека. Кроссфейда и маркера
    # `ТИШИНА` больше нет (§4.4 документа, единый retune-путь).
    retunes=$(count "retune spec#[0-9]+ -> spec#[0-9]+ \([A-Z][A-Z_]*\): приседание")
    retunes_by_reason=$(count "retune spec#[0-9]+ -> spec#[0-9]+ \($reason\): приседание")
    rebuilt=$(count "doRetuneOnWriter spec#[0-9]+: пакет перестроен")
    lifted=$(count "retune spec#[0-9]+: ПОДНЯТ")
    swap=$(count "beginOverlapSwitch: SWAP")          # остаться 0 — удалён
    recreated=$(count "recreateTrack")                 # остаться 0 — только смена SR
    ...
    printf '     приседаний=%s (этой причины=%s), пересборок на писателе=%s, подъёмов=%s, перекрытий=%s, recreateTrack=%s\n' \
        "$retunes" "$retunes_by_reason" "$rebuilt" "$lifted" "$swap" "$recreated"
    printf '     коалесценция: ожиданий=%s (серия=%s, одиночный=%s)\n' \
        "$waits" "$((waits - series))" "$series"

    # ПОРЯДОК ПЕРЕНАСТРОЙКИ: «приседание → пакет перестроен → ПОДНЯТ». Каждый
    # провал обязан закрыться подъёмом ТОГО ЖЕ трека И ТОЛЬКО после пересборки
    # пакета — ранний `ПОДНЯТ` без `пакет перестроен` означал бы щелчок (старт
    # на СТАРОМ PCM), а `ПОДНЯТ` без `приседания` — потерянный провал.
    local ord g w d early orphan
    ord=$(awk '
        /retune spec#[0-9]+ -> spec#[0-9]+ \([A-Z][A-Z_]*\): приседание/ {state=1; g++; next}
        /doRetuneOnWriter spec#[0-9]+: пакет перестроен/ {if (state==1) {state=2; w++} next}
        /retune spec#[0-9]+: ПОДНЯТ/ {
            if (state==2) {state=0; d++} else if (state==1) {early++; state=0} else orphan++; next}
        END {printf "%d %d %d %d %d", g, w, d, early, orphan+0}
    ' "$LOG")
    set -- $ord
    g="${1:-0}"; w="${2:-0}"; d="${3:-0}"; early="${4:-0}"; orphan="${5:-0}"
    printf '     порядок: приседаний=%s, пересборок=%s, подъёмов=%s\n' "$g" "$w" "$d"
    ...
    check "  второго трека не было: recreateTrack удалён" [ "$recreated" = "0" ]
    # retune-модель: каждый жест — это КОРОТКИЙ провал на одном треке.
    # Коалесценция (§4.3) здесь означает лишь то, что жесты внутри окна
    # ДОПИСЫВАЮТ цель одного провала, а не плодят новые, — поэтому число
    # провалов НЕ превышает числа жестов. В отличие от кроссфейда, тут НЕТ
    # верхней границы «≤ 2»: по CLI жесты прилетают реже окна провала
    # (~205 мс из-за латентности `am broadcast`), так что каждый жест
    # материализуется в свой провал — и это НОРМА, не дефект.
    check "  провалов не больше жестов (коалесценция не плодит)" \
        le "$retunes_by_reason" "$gestures"
    # Каждый пересобранный пакет обязан подняться, и НЕ раньше пересборки.
    # Допуск «−1» — последний провал может быть ещё в полёте (рампа вверх
    # ~125 мс) в момент снятия лога; сам по себе он не дефект (сторож
    # промолчал выше).
    check "  нет ПОДНЯТ без пересборки (порядок не нарушен)" [ "$lifted" -le "$rebuilt" ]
    check "  каждый пакет поднят (допуск 1 на летучий)" [ "$lifted" -ge $((rebuilt - 1)) ]
    # Коалесценция может СКРЫТЬ приседание (жест дописал цель уже летящего
    # провала), но не может породить лишних — поэтому g ≤ rebuilt+1.
    check "  приседаний не больше пересборок+1 (коалесценция не плодит лишних)" \
        [ "$g" -le $((rebuilt + 1)) ]
    check "  нет ПОДНЯТ без приседания (потерянный провал)" [ "$orphan" = "0" ]
