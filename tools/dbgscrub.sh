...
#   V2 сдвиг переживает перенастройку настроек (тот же путь, что правка точки);
...
#   V6 два скраба подряд внутри одного провала: итог = вторая цель;
...
#   DEVICE=192.168.199.165:5555 TOL=8 bash tools/dbgscrub.sh v7
#
...
# правка конфига, а именно она поднимает SETTINGS-перенастройку.
#
...
#     больше окна коалесценции провала). Многословная команда внутри
...
# Таймаут ожидания сходимости оси, сек (провал перенастройки ~0.3 с + генерация).
WAIT="${WAIT:-12}"
...
# ПОРЯДОК ПЕРЕНАСТРОЙКИ (§5.4.4 документа, ОДИН трек): «приседание →
# пересборка пакета на нити писателя → ПОДНЯТ на том же треке».
#
# Хэндофф удалён целиком: второго AudioTrack больше не существует, поэтому
# «гаснет → ТИШИНА → SWAP по нулю» искать нечего — этих строк нет в принципе.
# Остаётся проверить главное: каждый провал закрывается подъёмом ТОГО ЖЕ
# трека, причём ТОЛЬКО после того, как писатель реально перестроил пакет.
# Ранний `ПОДНЯТ` без `пакет перестроен` означал бы, что трек заиграл на
# СТАРОМ PCM (щелчок/скачок частоты), а «приседание» без подъёма — потерянный
# провал: звук остался в нуле навсегда.
expect_retune_order() {
    local label="$1" ord g w d early
    ord=$(awk '
        /retune spec#[0-9]+ -> spec#[0-9]+ \([A-Z][A-Z_]*\): приседание/ {state=1; g++; next}
        /doRetuneOnWriter spec#[0-9]+: пакет перестроен/ {
            if (state==1) {state=2; w++} next}
        /retune spec#[0-9]+: ПОДНЯТ/ {
            if (state==2) {state=0; d++} else if (state==1) {early++; state=0} next}
        END {printf "%d %d %d %d", g+0, w+0, d+0, early+0}
    ' "$LOG")
    set -- $ord
    g="${1:-0}"; w="${2:-0}"; d="${3:-0}"; early="${4:-0}"
    if [ "$g" -gt 0 ] && [ "$w" -eq "$g" ] && [ "$d" -eq "$g" ] && [ "$early" -eq 0 ]; then
        printf '  PASS  %s (приседаний=%s, пересборок=%s, подъёмов=%s)\n' \
            "$label" "$g" "$w" "$d"; PASSED=$((PASSED + 1))
    else
        printf '  FAIL  %s (приседаний=%s, пересборок=%s, подъёмов=%s, подъёмов без пересборки=%s)\n' \
            "$label" "$g" "$w" "$d" "$early"
        FAILED=$((FAILED + 1)); FAILED_LIST="$FAILED_LIST\n  - $SCEN: $label"
    fi
}
...
# предыдущей, уже проверенной перенастройки).
...
v1() {
    ...
    # Скраб — та же кривая, другой момент суток (§4.1, §4.2 документа). С
    # пятой волны это вообще не «переход», а перенастройка ОДНОГО трека:
    # вердикт когерентности удалён вместе с хэндоффом (решений по Δf больше
    # не принимается), а провал закрывается на том же самом AudioTrack.
    expect_log "перенастройка начата приседанием" \
        "retune spec#[0-9]+ -> spec#[0-9]+ \([A-Z][A-Z_]*\): приседание"
    expect_retune_order "скраб не создаёт второго трека: приседание → пересборка → ПОДНЯТ"
    expect_log "подъём на том же треке" "второго трека не было"
...
# ---------------------------------------------------------------- V2
v2() {
    banner "V2: сдвиг переживает перенастройку настроек"
    start_log v2
    restart_playback
    scrub_by 10800
    dump_axis
    ...
    # тот же: `updateConfig()` → `onSpecChanged(SETTINGS)` → перенастройка с новой
    ...
    stop_log
    check "ось осталась сдвинутой после перенастройки" t_axis "$LAST_SNAP"
    check "сдвиг не обнулился (+3 ч)" \
        lt "$(norm_delta "$(s_scrub "$LAST_SNAP")" 10800)" "$SCRUB_TOL"
    # Правка настройки НЕ меняет кривую (§4.2 документа). Раньше это значило
    # «Δf = 0 ⇒ когерентно ⇒ нулевое перекрытие»; теперь кроссфейда нет вовсе,
    # и важно другое: провал закрылся на том же треке, а сдвиг унаследован.
    expect_log "перенастройка начата приседанием" \
        "retune spec#[0-9]+ -> spec#[0-9]+ \([A-Z][A-Z_]*\): приседание"
    expect_retune_order "правка настройки не создаёт второго трека"
...
v5() {
    ...
    # её отсечёт, перенастройки не будет, и расхождение в сутки уедет в СЛЕДУЮЩИЙ
    # сценарий, где сторож честно заорет «INVARIANT НАРУШЕН» — на чужой ошибке.
    # Сначала часы (ось уезжает на сутки), тут же `pscrubreset` (перенастройка,
...
# ---------------------------------------------------------------- V6
v6() {
    banner "V6: два скраба подряд внутри одного провала"
    start_log v6
    restart_playback
    local now a b n
    now=$(s_now "$(snap)")
    ...
    # прилететь, пока первый провал ещё в полёте. Шторм гасится однослотовым
    # `pendingRetune` (latest-wins): второй жест лишь перезаписывает цель и
    # НЕ перезапускает таймер, поэтому «приседаний» в логе будет одно, а
    # `scrubTo` (уровень менеджера) — два.
...
v9() {
    ...
    # а на реальную ось звук вернёт только перенастройка. С одним пресетом `next`
